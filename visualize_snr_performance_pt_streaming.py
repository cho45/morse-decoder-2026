...
import config
from visualize_snr_performance import decode_ctc_output, calculate_cer, generate_random_text
...
class PyTorchStreamingEvaluator:
    ...
    def run_inference_streaming(self, mels: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.no_grad():
            for i in range(0, seq_len, chunk_size):
                ...
                (logits, signal_logits, _), states = self.model(chunk, states)
        ...
        return full_logits, full_signal_logits
    ...
    def evaluate_batch(self, texts: List[str], snr_db: float, wpm: int = 20, random_freq: bool = False) -> List[float]:
        ...
        for text in texts:
            ...
            logits, signal_logits = self.run_inference_streaming(mels)
...
def main():
    ...
    parser.add_argument("--output", type=str, default="diagnostics/snr_performance_pt_streaming.png")
