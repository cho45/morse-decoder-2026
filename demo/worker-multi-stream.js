...
class StreamManager {
    ...
    async init(modelBuffer, options = {}) {
        try {
            console.log("[StreamManager] Starting init...");
            console.log("[StreamManager] Creating ONNX session...");
            this.session = await ort.InferenceSession.create(modelBuffer);
            console.log("[StreamManager] ONNX session created");
            this.options = { ...this.options, ...options };
            console.log("[StreamManager] Options set:", this.options);
     ...
     * @returns {Array<Object>}
     */
    getSlotSignalHistory(slotId) {
        const slot = this.slots.get(slotId);
        if (!slot) {
            throw new Error(`Slot ${slotId} not found`);
        }
        return slot.inference.getSignalHistory();
    }
     ...
     * @returns {Array<Object>}
     */
    getSlotEvents(slotId) {
        const slot = this.slots.get(slotId);
        if (!slot) {
            throw new Error(`Slot ${slotId} not found`);
        }
        return slot.inference.getEvents();
    }
     ...
     */
    async waitForSlotProcessing(slotId) {
        const slot = this.slots.get(slotId);
        if (!slot) {
            throw new Error(`Slot ${slotId} not found`);
        }
        return await slot.inference.waitForProcessing();
    }
     ...
     * @returns {number}
     */
    getSlotFrameCount(slotId) {
        const slot = this.slots.get(slotId);
        if (!slot) {
            throw new Error(`Slot ${slotId} not found`);
        }
        return slot.inference.frameCount;
    }
     ...
     * @returns {number}
     */
    getSlotInferenceTime(slotId) {
        const slot = this.slots.get(slotId);
        if (!slot) {
            throw new Error(`Slot ${slotId} not found`);
        }
        return slot.inference.inferenceTime;
    }
     ...
     * @returns {boolean}
     */
    getSlotIsProcessing(slotId) {
        const slot = this.slots.get(slotId);
        if (!slot) {
            throw new Error(`Slot ${slotId} not found`);
        }
        return slot.inference.isProcessing;
    }
