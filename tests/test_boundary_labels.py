...
class TestBoundaryLabels(unittest.TestCase):
    def test_boundary_timing(self):
        ...
        # signal_labels: 0:Background/Space, 1:Dit, 2:Dah, 3:Inter-word space
        ...
        for i in range(space_start_idx, len(sig)):
            if bound[i] == 1.0:
                ...
                # 許容範囲を 15〜25 フレームとする (サンプリング誤差考慮)
                self.assertTrue(15 <= elapsed_frames <= 25, f"Boundary timing error: {elapsed_frames} frames from space start (Expected ~18)")
                # 境界は Inter-word (3) または 背景 (0) で発生しうる
                self.assertIn(sig[i], [0, 3], f"Boundary in unexpected signal class: {sig[i]}")
                # 信号のON区間(1, 2)と重なっていないことを厳密に確認
                self.assertNotIn(sig[i], [1, 2], f"Boundary overlapped with signal! Class: {sig[i]}")
                found_boundary = True
                break
        
        self.assertTrue(found_boundary, "Boundary label not found after character")

    def test_no_boundary_in_signal(self):
        # 信号のON区間(Dit/Dah)には Boundary が立たないことを確認
        text = "M" # --
        ...
        on_indices = np.where((sig == 1) | (sig == 2))[0]
        for idx in on_indices:
            self.assertEqual(bound[idx], 0.0, f"Boundary must NOT exist in signal! Class: {sig[idx]}")
