...
- **信号電力 ($C$):** モールス信号が **ON（マーク）の状態** における正弦波の平均電力。
  - 振幅 $A$ の正弦波の場合、$` C = A^2 / 2 `$ です。
- **雑音電力密度 ($N_0$):** 加法性ホワイトガウスノイズ (AWGN) の 1Hz あたりの電力。
- **搬送波対雑音電力密度比 ($C/N_0$):** $` C/N_{0(\text{dB-Hz})} = 10 \log_{10} (C / N_0) `$
- **本プロジェクトの SNR ($\text{SNR}_{2500}$):** 基準帯域幅 $B = 2500\text{Hz}$ を用いた以下の値。
  - $` \text{SNR}_{2500} = C/N_{0(\text{dB-Hz})} - 10 \log_{10}(2500) \approx C/N_{0(\text{dB-Hz})} - 34.0 `$

この定義により、サンプリングレート $F_s$ が変更されても、モデルが実際に見る周波数帯域内のノイズ密度は一定に保たれます。また、FT8 などの他のデジタルモード（2500Hz 基準が一般的）との比較も容易になっています。

### SNR 換算の目安

CW オペレータが一般的に使用する 500Hz 帯域フィルタでの聞こえ方と比較する場合、以下の換算が可能です。

500Hz 帯域換算 (CW フィルタ相当):
```math
\text{SNR}_{500} \approx \text{SNR}_{2500} + 10 \log_{10}\left(\frac{2500}{500}\right) \approx \text{SNR}_{2500} + 7.0 \text{dB}
```

例：本プロジェクトでの $\text{SNR}_{2500} = -10 \text{dB}$ は、CW フィルタを通した環境では $\text{SNR}_{500} = -3 \text{dB}$（ノイズより信号がわずかに弱い状態）に相当します。
